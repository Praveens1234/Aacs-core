...
> **Autonomous Agent Coding System** — A multi-agent framework where 23 specialized AI agents collaborate to autonomously plan, build, test, and deliver production software.

![Build](https://img.shields.io/badge/build-passing-brightgreen)
![Tests](https://img.shields.io/badge/tests-696%20passed-success)
...
- **Memory Engine** — Three-tier memory: per-agent working memory (dict), episodic memory (SQLite + FTS5 + ChromaDB vector search), and project memory (JSON) for conventions and decisions
...
- 🤖 **23 Specialized Agents** — Each with focused role descriptions, quality checklists, and permitted toolsets, ensuring clear separation of concerns
...
- 🧠 **Three-Tier Memory** — Working memory (per-agent dict), Episodic memory (SQLite + FTS5 + ChromaDB), Project memory (JSON)
...
- 🌐 **WebUI** — FastAPI server with WebSocket real-time events, token-based authentication, file browser, task management
...
- 🔧 **53 Registered Tools** — Build, test, lint, security scan, code analysis, git operations, monitoring, search, knowledge queries
...
│   ├── knowledge_agent.py    # Knowledge graph queries and updates
│   └── memory_keeper.py      # Memory consolidation and management
│
...
│
├── webui/                    # Web interface
│   ├── server.py             # FastAPI app — REST API, WebSocket, auth, static files
...
The project includes 696 tests across 11 test files covering all core modules:
